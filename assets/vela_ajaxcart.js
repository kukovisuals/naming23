...
(window.XMLHttpRequest.prototype.open = cartUpdateListener),
    ...
    (ShopifyAPI.addItemFromForm = async function (a, d, e) {
        ...
        let bndlId = document.querySelector('#eby-bundle-bras23');
        ...
        if(getBndlQuickId.includes('product-actions') && bndlConditional && isABundleProd){
            ...
            braVariants = bndlId.dataset.bundlepdp;  
        } else if(bndlId){
            braVariants = bndlId.dataset.bundlepdp; 
        }
        ...
        /*
            ...
            ********************************************************
        */
        else if(typeof braVariants === 'string'){

            // check if bundle has different colors
            const braPantySecondPdp = bndlId.dataset.bundlepdp2;
            const bundleSetProductId = bndlId.dataset.mainids;
            ...
            if (braPantySecondPdp.length > 10) {
                const itemSend1 = await mainBundleBralette(braVariants, 1, priceProperty[0], bundleSetProductId);
                const itemSend2 = await mainBundleBralette(braPantySecondPdp, 1, priceProperty[1], bundleSetProductId);
                mainBundleSet(itemSend1, itemSend2);
            } else { 
                const itemSend1 = await mainBundleBralette(braVariants, 2, priceProperty, bundleSetProductId);
            ...
            async function sendBundleSet(itemArr){
                ...
                try {
                    ...
                    postData.items.forEach( (d,i) => {
                        ...
                        if( i > 0 ){
                          gaTrackingObj.productTitleTwo = d.title
                        } else {
                          gaTrackingObj.productTitleOne = d.title
                        }
                    });
            ...
            function itemPayload(...items){
                ...
                const typePdp = bndlId.dataset.typepdp;
            ...
            function mainBundleBralette(bundleIds, n, mixType, _ids){
                ...
                // toDo - get title and price of product with discount
                if (!!window.productsSiblings){
                  var prodUriIndex = window.location.pathname.split('/').length - 1;
                  var prodUriData = window.location.pathname.split('/');
                  var prodParentData = {
                    title : window.productsSiblings[prodUriData[prodUriIndex]].title,
                    price : window.productsSiblings[prodUriData[prodUriIndex]].price
                  };

                  var item = {
                      quantity: n, 
                      variantId : _ids,
                      title: prodParentData.title,
                      price: prodParentData.price,
                      id: idSend, 
                      type: mixType
                  };
                } else {
      ...
      function bundleSetGaTracking(b){
          const newPrice = +document.querySelector('.nrml-price23 ').innerText.split('$')[1];
          ...
          gtag('event', 'add_to_cart__bundleSet', {
              ...
              page_title: document.title,
              item_id: b.itemId,
              item_nameOne: b.productTitleOne,
              item_nameTwo: b.productTitleTwo,
              item_properties: b.property
          });

          jQuery.getJSON(window.location.pathname + '.js', function(bundleSetProduct) {
              console.log('data of set product ::', bundleSetProduct);

              gtag("event", "add_to_cart", {
                  currency: "USD",
                  value: parseInt(Shopify.formatMoney(bundleSetProduct.price).replace("$", "")),
                  page_location: window.location.pathname, 
                  page_title: document.title,
                  event_category: bundleSetProduct.title  + " - " + parseInt(Shopify.formatMoney(bundleSetProduct.price).replace("$", "")),
                  event_label: bundleSetProduct.type === "Season Set" ? 'atc_type-set' : 'atc_type-bundle',
                  items: [
                      {
                          item_id: bundleSetProduct.id,
                          item_name: bundleSetProduct.title,
                          affiliation: bundleSetProduct.vendor,
                          index: 0,
                          item_category: bundleSetProduct.type,
                          item_variant: !!document.querySelectorAll('.set-selected-variants')[0] ? document.querySelectorAll('.set-selected-variants')[0].value + "::" + document.querySelectorAll('.set-selected-variants')[1].value : document.querySelector('.swatch-element.active').getAttribute('data-value'),
                          price: parseInt(Shopify.formatMoney(bundleSetProduct.price).replace("$", "")),
                          currency: "USD",
                          quantity: 1,
                      },
                  ],
    ...
    return (
                ...
                void 0 != window.ShopifyAnalytics.meta.page.customerId ? ((d = !0), (b = 0)) : (d = "function" == typeof checkBalanceValue && 0 >= checkBalanceValue(a.total_price / 100)),
                    ...
                    (
                      ...
                      c = {
                        ...
                        trueEcommSubtotalWithShipping: !!hasStandardShipping ? Shopify.formatMoney((a.total_price - l - b) + (600 + totalOtherSavingsFinal), settings.moneyFormat).replace(".00", "") : Shopify.formatMoney((a.total_price - l - b) + ( + totalOtherSavingsFinal), settings.moneyFormat).replace(".00", ""),
        ...
        (adjustCart = function () {
            function grabSecondBndlSet(line, sync){
                ...
                $body.on("click", ".qtyAdjust", function (qntEvent) {
                    var f = $(this),
                        c = f.data("line"),
                        e = f.siblings(".qtyNum"),
                        h = e.data("prop"),
                    ...
                    if ((f.hasClass("velaQtyPlus") ? (b += 1) : ((b -= 1) <= 0 && (b = 0), !0 == d && (d = !1)), $(".ajaxCartProduct.related").length > 0)) {
                        ...
                        if(f.closest(".ajaxCartProduct").hasClass("isBundleSet")){
                            ...
                            if( typeof dataProp === 'string'){ 
                                const variantIds = dataProp.split('::')
                                updates = {
                                    [variantIds[0]]: b, // Set the quantity of line item 1 to 2
                                    [variantIds[1]]: b  // Set the quantity of line item 2 to 1
                                };    
                            } else{ 
                                if (this.classList.contains("velaQtyMinus")) { 
                                    updates = {
                                        [dataProp]: b - 1, 
                                    };  
                                } else {
                            ...
                            $.ajax({
                                ...
                                error: function(response) {
                                    console.log('Failed to update cart: ' + response.responseText);
                                }
                            });
                        } else {
